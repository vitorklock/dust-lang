...
// Type of an expression; "sign" is the internal type of text literals (never a variable type)
...
export class SemanticAnalyzer {
    ...
    private checkFlowInto(source: ExprType, dest: A.DustType, name: string, line: number): void {
        ...
        if (source === "dust" && dest === "comparator") return; // promotion int -> float
    ...
    private condition(e: A.Expr): void {
        const t = this.expr(e);
        if (e.kind === "Compare") return; // Compare is already logical
        if (t !== "lever") {
            throw new SemanticError(
